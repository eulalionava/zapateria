...
import { ModalAddCardPont } from "@/src/modals/ModalAddCardPoint";

export default function VentaPage(){

  const router = useRouter()
  const { register, handleSubmit, formState: { errors }, reset } = useForm<GetProduct>();
  const [activeModalCardPoint, setActiveModalCardPoint] = useState(false);
  ...
  const excecuteSale = async(showAlert:boolean)=>{
    ...
    if(showAlert) Swal.fire(response, '', 'success')
  ...
  const cuestionGeneratePoints = ()=>{
    ...
    Swal.fire({
      title: "¿Deseas generar puntos por esta venta?",
      showDenyButton: true,
      confirmButtonText: "Venta",
      denyButtonText: `Generar puntos`,
    }).then((result) => {
      if (result.isConfirmed){
        excecuteSale(true)
        setActiveModalCardPoint(false)
      }
      else if (result.isDenied) setActiveModalCardPoint(true)
    });
  ...
  return (
    <>
      { activeModalCardPoint && 
        <ModalAddCardPont 
          desactivar={()=>setActiveModalCardPoint(false)} 
          excecuteSale={()=>excecuteSale(false)} 
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4">
                    ...
                    <button 
                      ...
                      onClick={cuestionGeneratePoints}
                    >
                      {loading ?(
